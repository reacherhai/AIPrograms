...
def extract_cnn_feature(model, inputs1,inputs2, output_feature=None):
    model.eval()
    inputs1 = to_torch(inputs1)
    inputs1 = Variable(inputs1)#, volatile=True)
    inputs2 = to_torch(inputs2)
    inputs2 = Variable(inputs2)#, volatile=True)
    outputs = model(inputs1,inputs2, output_feature)
...
def extract_features_pair(model, data_loader, print_freq=1, output_feature=None):
    ...
    for k, (imgs1, imgs2, ids, _) in enumerate(data_loader):
        ...
        ######method1############
        #imgs=imgs1-imgs2
        outputs = extract_cnn_feature(model, imgs1, imgs2, output_feature)
        #######method2#############
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, gallery_loader, query, gallery, output_feature=None, rerank=False):
        save1='single_data/ranking4.mat'
        save2='single_data/dist4.mat'
...
class EvaluatorRelations(object):
    ...
    def evaluate(self, query_loader, gallery_loader, query, gallery, output_feature=None, rerank=False):
        save1='pair_data/ranking6.mat'
        save2='pair_data/dist6.mat'
